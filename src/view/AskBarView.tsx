...
import nexusLogo from "../../icon.png";
...
const HELP_ICON = (
  ...
  >
    ...
    <path
      d="M9 9a3 3 0 116 0c0 2-3 3-3 3m0 4h.01"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    ...
    >
      {selectedText && (
        <div className="px-4 pt-3 pb-0">
          <p className="rounded-2xl border border-white/8 bg-white/4 px-3 py-2 italic text-xs text-text-secondary select-text whitespace-pre-wrap">
            &ldquo;
            {formatQuotedText(
              selectedText,
      ...
      <div className="relative">
        <div className="flex items-center w-full px-4 py-3 gap-2.5">
          <img
            src={nexusLogo}
            alt="Nexus AI"
            className={`shrink-0 transition-all duration-300 ease-out ${
              isChatMode
                ? "w-7 h-7 rounded-xl shadow-[0_8px_24px_-12px_rgba(255,141,92,0.65)]"
                : "w-11 h-11 rounded-2xl shadow-[0_16px_30px_-16px_rgba(255,141,92,0.7)]"
            }`}
            draggable={false}
          />
          ...
          {!isChatMode && onHistoryOpen && (
            <button
              ...
              className="shrink-0 w-8 h-8 flex items-center justify-center rounded-xl text-text-secondary hover:text-text-primary hover:bg-white/8 transition-colors duration-150 cursor-pointer outline-none"
            >
              {HISTORY_ICON}
            </button>
          )}
          ...
          <div className="relative flex-1 min-w-0">
            ...
            <div
              ref={mirrorRef}
              aria-hidden="true"
              className="absolute inset-0 pointer-events-none bg-transparent text-text-primary text-sm py-2.5 px-1 leading-relaxed whitespace-pre-wrap break-words overflow-hidden"
            >
            ...
            <textarea
              ...
              rows={1}
              placeholder={
                isChatMode ? "Reply to Nexus AI..." : "Ask Nexus AI anything..."
              }
              className="relative w-full bg-transparent border-none outline-none text-transparent text-sm placeholder:text-text-secondary py-2.5 px-1 disabled:opacity-50 resize-none leading-relaxed"
          ...
          {isAtMaxImages ? (
            <Tooltip label="Maximum 3 images attached">
              <button
                ...
                className="shrink-0 w-8 h-8 flex items-center justify-center rounded-xl text-text-secondary transition-colors duration-150 disabled:opacity-40 disabled:cursor-default cursor-pointer"
              >
                {CAMERA_ICON}
              </button>
            </Tooltip>
            ...
            <Tooltip label="Take a screenshot">
              <button
                ...
                disabled={isBusy}
                aria-label="Take screenshot"
                className="shrink-0 w-8 h-8 flex items-center justify-center rounded-xl text-text-secondary hover:text-text-primary hover:bg-white/8 transition-colors duration-150 disabled:opacity-40 disabled:cursor-default cursor-pointer"
              >
                {CAMERA_ICON}
              </button>
            </Tooltip>
          ...
          <Tooltip label="Settings">
            <button
              ...
              className="shrink-0 w-8 h-8 flex items-center justify-center rounded-xl text-text-secondary hover:text-text-primary hover:bg-white/8 transition-colors duration-150 cursor-pointer"
            >
              {SETTINGS_ICON}
            </button>
          </Tooltip>
          ...
          <motion.button
            ...
            className={`shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center transition-colors duration-200 ${
              isBusy
                ? "stop-btn-ring bg-red-500/10 text-red-400 cursor-pointer"
                : canSubmit
                  ? "bg-primary text-neutral shadow-[0_14px_28px_-16px_rgba(255,141,92,0.85)] cursor-pointer"
        ...
        {!isChatMode && (
          <div className="px-4 pb-3 flex items-center gap-2 overflow-hidden">
            <span className="rounded-full border border-white/8 bg-white/4 px-2.5 py-1 text-[10px] font-medium tracking-[0.18em] text-text-secondary uppercase">
              Ctrl Ctrl
            </span>
            <span className="rounded-full border border-white/8 bg-white/4 px-2.5 py-1 text-[11px] text-text-secondary">
              /web search
            </span>
            <span className="rounded-full border border-white/8 bg-white/4 px-2.5 py-1 text-[11px] text-text-secondary">
              /shell ls -la
